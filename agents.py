...
class Agent:
    ...
    def allowed_directions(self, pos, world):
        return [direction for direction in (left, right, up, down) if world.no_obstacle(pos+direction)]
    def path_to_actions(self, path):
        #directions = np.diff(path)
...
class TravelingSalesmanBFAgent(AStar1StepAgent):
    def make_plan(self, world):
        ...
        perms = [list(t) for t in list(itertools.permutations(range(1,len(dirts)+1)))]#yes im stupid
        p_costs, p_plans = [], []
        for perm in perms:
            perm.insert(0,0)
            p_cost = 0
        ...
        for departure, destination in zip(best_perm[:-1],best_perm[1:]):
            best_plan += self.path_to_actions(path_matrix[destination,departure])+[self.suck]
        self.plan = best_plan
